...
var (
	...
	tclAllocProc            *windows.Proc
	tclDll                  *windows.DLL
	tclEventCallback        = windows.NewCallbackCDecl(tclEventProc)
	tclGetCurrentThreadProc *windows.Proc
	tclThreadAlertProc      *windows.Proc
	tclThreadQueueEventProc *windows.Proc
	tkDll                   *windows.DLL
)
...
func lazyInit() {
	...
	threadID, _, _ = tclGetCurrentThreadProc.Call()
...
func bindLibs(cacheDir string) {
	...
	if tclAllocProc, Error = tclDll.FindProc("Tcl_Alloc"); Error != nil {
		return
	}

	if tclThreadQueueEventProc, Error = tclDll.FindProc("Tcl_ThreadQueueEvent"); Error != nil {
		return
	}

	if tclThreadAlertProc, Error = tclDll.FindProc("Tcl_ThreadAlert"); Error != nil {
		return
	}

	if tclGetCurrentThreadProc, Error = tclDll.FindProc("Tcl_GetCurrentThread"); Error != nil {
		return
	}
...
func postEvent(f func()) (err error) {
	event, _, _ := tclAllocProc.Call(unsafe.Sizeof(goTclEvent{}))
	if event == 0 {
		return fmt.Errorf("OOM")
	}
	...
	*(*goTclEvent)(unsafe.Pointer(event)) = goTclEvent{
	...
	tclThreadQueueEventProc.Call(threadID, event, tcl_queue_tail)
	tclThreadAlertProc.Call(threadID)
	return nil
}

// typedef int (Tcl_EventProc) (Tcl_Event *evPtr, int flags);
func tclEventProc(evPtr uintptr, flags int32) uintptr {
