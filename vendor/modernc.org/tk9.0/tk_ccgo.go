...
var (
	...
	tclEventCallback = __ccgo_fp(tclEventProc)
)
...
`, zf1, tclLibMountPoint, zf2, tkLibMountPoint)); Error != nil {
	...
	threadID = libtcl.XTcl_GetCurrentThread(tls)
...
func postEvent(tls *libc.TLS, f func()) (err error) {
	event := libtcl.XTcl_Alloc(tls, libc.Tsize_t(unsafe.Sizeof(goTclEvent{})))
	if event == 0 {
	...
	*(*goTclEvent)(unsafe.Pointer(event)) = goTclEvent{
	...
	libtcl.XTcl_ThreadQueueEvent(tls, threadID, event, tcl_queue_tail)
	libtcl.XTcl_ThreadAlert(tls, threadID)
	return nil
}

// typedef int (Tcl_EventProc) (Tcl_Event *evPtr, int flags);
func tclEventProc(tls *libc.TLS, evPtr uintptr, flags int32) int32 {
