...
//go:build (linux && (amd64 || arm64)) || (darwin && (amd64 || arm64)) || (freebsd && (amd64 || arm64)) || (openbsd && amd64)
...
var (
	...
	tclAllocProc            uintptr
	tclBinHandle            uintptr
	tclEventCallback        = purego.NewCallback(tclEventProc)
	tclGetCurrentThreadProc uintptr
	tclThreadAlertProc      uintptr
	tclThreadQueueEventProc uintptr
	tkBinHandle             uintptr
)
...
func bindLibs(cacheDir string) {
	...
	if tclAllocProc, Error = purego.Dlsym(tclBinHandle, "Tcl_Alloc"); Error != nil {
		return
	}

	if tclThreadQueueEventProc, Error = purego.Dlsym(tclBinHandle, "Tcl_ThreadQueueEvent"); Error != nil {
		return
	}

	if tclThreadAlertProc, Error = purego.Dlsym(tclBinHandle, "Tcl_ThreadAlert"); Error != nil {
		return
	}

	if tclGetCurrentThreadProc, Error = purego.Dlsym(tclBinHandle, "Tcl_GetCurrentThread"); Error != nil {
		return
	}
	...
	threadID, _, _ = purego.SyscallN(tclGetCurrentThreadProc)
...
func postEvent(f func()) (err error) {
	event, _, _ := purego.SyscallN(tclAllocProc, unsafe.Sizeof(goTclEvent{}))
	if event == 0 {
		return fmt.Errorf("OOM")
	}

	*(*goTclEvent)(unsafe.Pointer(event)) = goTclEvent{
		e: tclEvent{
			proc: tclEventCallback,
		},
		handle: uintptr(cgo.NewHandle(f)),
	}

	purego.SyscallN(tclThreadQueueEventProc, threadID, event, tcl_queue_tail)
	purego.SyscallN(tclThreadAlertProc, threadID)
	return nil
}

// typedef int (Tcl_EventProc) (Tcl_Event *evPtr, int flags);
func tclEventProc(evPtr uintptr, flags int32) int32 {
	handle := cgo.Handle((*goTclEvent)(unsafe.Pointer(evPtr)).handle)

	defer handle.Delete()

	switch x := handle.Value().(type) {
	case func():
		x()
	default:
		panic(todo("unsupported event type: %T", x))
	}

	// This tells Tcl that the event has been processed and that Tcl should free
	// the event struct itself using Tcl_Free. This matches our Tcl_Alloc call.
	return 1
}
