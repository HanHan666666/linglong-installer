...
func XXauLockAuth(tls *libc.TLS, file_name uintptr, retries int32, timeout int32, dead int32) (r int32) {
	...
	var _ /* statb at bp+2052 */ Tstat
	_, _ = creat_fd, now
	creat_fd = -int32(1)
	if libc.Xstrlen(tls, file_name) > uint32(1022) {
		return int32(m_LOCK_ERROR)
	}
	libc.X__builtin_snprintf(tls, bp, uint32(1025), __ccgo_ts+37, libc.VaList(bp+2208, file_name))
	libc.X__builtin_snprintf(tls, bp+1025, uint32(1025), __ccgo_ts+42, libc.VaList(bp+2208, file_name))
	if libc.Xstat(tls, bp, bp+2052) != -int32(1) {
		now = libc.Xtime(tls, libc.UintptrFromInt32(0))
		/*
		 * NFS may cause ctime to be before now, special
		 * case a 0 deadtime to force lock removal
		 */
		if dead == 0 || now-(*(*Tstat)(unsafe.Pointer(bp + 2052))).Fst_ctim.Ftv_sec > int64(dead) {
			libc.Xremove(tls, bp)
