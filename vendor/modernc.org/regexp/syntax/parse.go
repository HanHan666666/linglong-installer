...
const (
	...
	DisableOptimizations                   // can help constructing a DFA later
...
func (p *parser) optimizations() bool { return p.flags&DisableOptimizations == 0 }
...
func (p *parser) swapVerticalBar() bool {
	...
	n := len(p.stack)
	if n >= 3 && p.stack[n-2].Op == opVerticalBar && isCharClass(p.stack[n-1]) && isCharClass(p.stack[n-3]) && p.optimizations() {
		re1 := p.stack[n-1]
		re3 := p.stack[n-3]
